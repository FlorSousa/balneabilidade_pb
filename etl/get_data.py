...
def extract_url_list(url):
    ...
    return list(map(lambda x: x.find("a").get("href"),filter(get_url_from_row, tabela)))

def extract_name(url,i):
    patterns = [r'(?<=-)\d+(?=_)', r'(?<=-)\d+(?=-)', r'(?<=_)\d+(?=.)', r'(?:-|Balneabilidade)(\d+)_', r'(?<=-)\d+(?=_)', r'(?<=-)\d+(?=.)']
...
if __name__ == '__main__':
    ...
    name = f"DOWNLOAD {time.strftime('%X %d-%m-%y')}.log"
